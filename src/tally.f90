...
!print *, i,':',reduced_tal(i)%sum0
    ...
    call MPI_REDUCE(n_abs, reduced_n_abs, 1, MPI_INTEGER, MPI_SUM, 0, MPI_COMM_WORLD, mpi_err)
    if (mpi_err /= MPI_SUCCESS) then
       print *, "Failed to reduce n_abs."
       stop
    end if
    call MPI_REDUCE(n_fiss, reduced_n_fiss, 1, MPI_INTEGER, MPI_SUM, 0, MPI_COMM_WORLD, mpi_err)
    if (mpi_err /= MPI_SUCCESS) then
       print *, "Failed to reduce n_fiss."
       stop
    end if
