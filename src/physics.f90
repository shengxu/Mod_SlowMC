...
!===============================================================================
  ...
  function sample_region() result(region)
    ...
    use global, only: n_materials,Dancoff,res_iso,radius,mat,neut !, eidx !commented out by S. Xu
...
!     &            (sum(mat(2)%totalxs(eidx,:))*mat(2)%vol)
      ...
      Pmf = Pfm * (sum(mat(1)%xs_Total_brdn)*mat(1)%vol) /                   &
...
!===============================================================================
  ...
  function sample_isotope(region) result(isoidx)
    ...
    use global, only: mat !,eidx ! commented out by S. Xu
...
! Added by S. Xu (Apr. 2012)
    pmf(2:size(pmf)) = mat(region)%xs_Total_brdn/sum(mat(region)%xs_Total_brdn)
...
! Added by S. Xu (May. 2012) for debug
! write(11, *) cdf
! write(11, *) rn, isoidx
...
! Added by S. Xu (Apr. 2012)
    pmf = (/0.0_8,mat(region)%isotopes(isoidx)%xs_fiss_brdn,                  &
   &              mat(region)%isotopes(isoidx)%xs_capt_brdn,                  &
                  mat(region)%isotopes(isoidx)%xs_scat_brdn/)
...
! write(11, '(5e19.8e3)') rn, cdf
! write(11, '(a8, 2x, i3)') mat(region)%isotopes(isoidx)%name, reactid
! print *, mat(region)%isotopes(isoidx)%name, reactid
