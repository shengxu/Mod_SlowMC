...
!    write(*,101) 'k-inf (nufission):',col_kinf_mean_2,col_kinf_std_2
!    write(*,*) 'total reaction:', reaction_tally(1)
!    write(*,*) 'absorption:', reaction_tally(2)
!    write(*,*) 'scattering:', reaction_tally(3)
...
!print *, 'one output file = ', trim(output_path)//'log_'//trim(output_filename)//'.out'
    ! rewrite results to outputfile
    open(112,file=trim(output_path)//'log_'//trim(output_filename)//'.out', status="unknown")
        ! write results header
        ...
        ! write k-inf information
        write(112,101) 'k-inf (analog):',ana_kinf_mean,ana_kinf_std
        write(112,101) 'k-inf (coll):',col_kinf_mean,col_kinf_std    
    close(112)

    ! open up output hdf5 file
!    call h5fcreate_f("output.h5",H5F_ACC_TRUNC_F,hdfile,error)
    call h5fcreate_f(trim(output_path)//trim(output_filename)//"_out.h5",H5F_ACC_TRUNC_F,hdfile,error)
